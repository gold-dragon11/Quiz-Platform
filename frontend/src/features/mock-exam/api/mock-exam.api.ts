...
import type { MockExamAttempt, MockExamSpec } from '@/features/mock-exam/types/mock-exam.types';
...
export const mockExamApi = {
  /**
   * POST /quiz/mock-exam/start — the subject is the only choice there is.
   * Everything else about a sitting is fixed by the specification, because a
   * mock a student can configure is just a quiz with a longer name.
   */
  async start(subjectId: string): Promise<QuizSessionMetadata> {
    const { data } = await apiClient.post<QuizSessionMetadata>('/quiz/mock-exam/start', {
      subjectId,
    });
    return data;
  },
  ...
  async spec(subjectId: string): Promise<MockExamSpec> {
    const { data } = await apiClient.get<MockExamSpec>('/quiz/mock-exam/spec', {
      params: { subjectId },
    });
    return data;
  },
