...
- selectedAnswer — shape depends on the question type:
  ...
  - Numeric: `{ "numericAnswer": "12,5" }` — a string or a number. Compared by value, not by spelling: "12,5", "12.50" and 12.5 are the same answer, and the comma is accepted because the paper writes decimals with one. Text that is not a number — an empty field, a lone minus sign mid-typing — is stored as a wrong answer rather than rejected, because every keystroke autosaves. Only a wrong shape (an object, an array, a missing key) is `400`.
